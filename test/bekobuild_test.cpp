...
TEST_F(BekobuildSimplePackage, Build) {
  ...
  ASSERT_STREQ("./configure", string_vector_at(bekobuild->build, 0));
  ASSERT_STREQ("make", string_vector_at(bekobuild->build, 1));
}

TEST_F(BekobuildSimplePackage, Package) {
  ASSERT_EQ(1, bekobuild->package->size);
  ASSERT_STREQ("make install", string_vector_at(bekobuild->package, 0));
}
