...
protected:
  ...
  void push_fake_nodes() {
    for (int i = 0; i < 3; ++i) {
      ...
      node_set_value(node, fake_items[i]);
      vector_push(v, node);
    }
  }
};
...
TEST_F(VectorTest, CheckNodes) {
  push_fake_nodes();
  EXPECT_STREQ("item-0", node_to_string(vector_at(v, 0)));
  EXPECT_STREQ("item-1", node_to_string(vector_at(v, 1)));
  EXPECT_STREQ("item-2", node_to_string(vector_at(v, 2)));
}
